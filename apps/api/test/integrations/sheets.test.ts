...
describe('sheets.appendRows', () => {
  it('appends to the pinned tab with USER_ENTERED', async () => {
  ...
  it('sanitizes a googleapis error — never leaks credentials', async () => {
    ...
    // The thrown error must not carry the raw payload / secret anywhere.
    expect(JSON.stringify({ m: err.message, d: err.details })).not.toContain('PRIVATE-SECRET-123')
  })
})
