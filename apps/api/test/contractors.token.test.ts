...
import { generateLinkToken, parseLinkToken, validateLinkToken } from '../src/contractors/token'
...
async function makeContractor() {
  ...
  const c = await app.prisma.contractor.create({
    data: { landlordId, name: 'Joe', contact: 'x', tokenLookupId: link.lookupId, tokenHash: link.tokenHash },
  })
  return { contractor: c, link }
}

beforeAll(async () => {
  await app.ready()
  const l = await app.prisma.user.create({
    data: { email: `tok-${Date.now()}@example.com`, role: 'LANDLORD', passwordHash: await hashPassword('x') },
  })
  landlordId = l.id
})
afterAll(async () => {
...
describe('U4 link token', () => {
  it('a freshly generated token validates to its contractor + landlord', async () => {
    const { contractor, link } = await makeContractor()
    const resolved = await validateLinkToken(app.prisma, link.plaintext)
    expect(resolved).toEqual({ contractorId: contractor.id, landlordId })
  })

  it('stores only the hash, never the plaintext secret (hashed at rest)', async () => {
    const { contractor, link } = await makeContractor()
    const row = await app.prisma.contractor.findUniqueOrThrow({ where: { id: contractor.id } })
  ...
  it('a revoked token fails identically to a never-existing one (uniform null)', async () => {
    const { contractor, link } = await makeContractor()
    await app.prisma.contractor.update({ where: { id: contractor.id }, data: { revokedAt: new Date() } })
  ...
  it('a right lookupId with the wrong secret fails the hash compare', async () => {
    const { link } = await makeContractor()
