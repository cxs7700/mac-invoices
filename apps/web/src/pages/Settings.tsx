  ...
  return (
    <Section title={t('settings.profile.title')}>
      <div className="space-y-3">
        ...
        {update.isSuccess && (
          <p className="text-sm text-status-paid-foreground">{t('settings.profile.saved')}</p>
        )}
        {errOf(update.error) && (
    ...
    <Section title="Google Sheets connection">
      {isPending ? (
        ...
        <div className="space-y-3">
          ...
          {status?.serviceAccountEmail && (
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Add it as an Editor on the spreadsheet.
              </p>
            </div>
          )}
          <div>
            <label htmlFor="sheetId" className="text-sm font-medium text-foreground">
          ...
          {test.isSuccess && (
            <p className="text-sm text-status-paid-foreground">Connection works.</p>
          )}
          {errOf(test.error) && (
            <p className="text-sm text-destructive" role="alert">
              {errOf(test.error)}
          ...
          <div className="flex gap-2">
            <Button
              ...
              onClick={() => save.mutate({ spreadsheetId: value.trim() })}
            >
